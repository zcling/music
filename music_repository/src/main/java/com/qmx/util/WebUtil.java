...
import com.qmx.constants.LoginInfo;
...
public final class WebUtil {
     ...
     * 获取当前用户
     */
    public static final LoginInfo getCurrentUser() {
        Subject currentUser = SecurityUtils.getSubject();
        if (null != currentUser) {
            try {
                Session session = currentUser.getSession();
                if (null != session) {
                    return (LoginInfo) session.getAttribute(Constants.CURRENT_USER);
                }
    ...
    public static final String getCurrentUserRole() {
        try {
            ...
            if (null != currentUser) {
                Session session = currentUser.getSession();
                if (null != session) {
                    LoginInfo principal = (LoginInfo) currentUser.getPrincipal();
                    if (principal != null) {
